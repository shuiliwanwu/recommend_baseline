...
################################## PREPARE DATASET ##############################
itemKb = pd.read_pickle("itemKb.pkl")
...
if os.path.exists('./model.pt'):
	...
	model = NCF(user_size, item_size, FLAGS.embed_size, FLAGS.dropout,itemKb)
	model.cuda()
...
#		print(loss)        
	...
	print("MAP is %.3f\tMRR is %.3f\tHit ratio is %.3f\tNdcg is %.3f" %(np.mean(AP),np.mean(MRR),np.mean(HR), np.mean(NDCG)))
